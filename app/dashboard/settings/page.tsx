...
function ModelsTab() {
  ...
  const [loading, setLoading] = useState(true);
  const [configured, setConfigured] = useState<{ google: boolean; anthropic: boolean }>({ google: false, anthropic: false });
  ...
  useEffect(() => {
    fetch("/api/settings/models")
      ...
      .then((d) => {
        setModels(d.models ?? []);
        setConfigured(d.configured ?? { google: false, anthropic: false });
      })
      .catch(() => { })
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            { label: "OpenAI", key: "OPENAI_API_KEY", ok: !!process.env.NEXT_PUBLIC_HAS_OPENAI, color: "#10a37f" },
            { label: "Pinecone", key: "PINECONE_API_KEY", ok: false, color: "#6366f1" },
          ].map((kv) => (
...
function ToolsTab() {
  ...
  const [apiTools, setApiTools] = useState<{ id: string; name: string; url: string; method: string }[]>([]);
  const [showAddAPI, setShowAddAPI] = useState(false);
  const [newTool, setNewTool] = useState({ name: "", url: "", method: "GET" });
  ...
  function addAPITool() {
    ...
    setNewTool({ name: "", url: "", method: "GET" });
    setShowAddAPI(false);
  }
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        {apiTools.length === 0 && !showAddAPI ? (
          ...
          <div className="space-y-2">
            {apiTools.map((tool) => (
              <div key={tool.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl border"
                style={{ background: "rgba(34,211,238,0.05)", borderColor: "rgba(34,211,238,0.2)" }}>
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded"
                  style={{ background: "rgba(34,211,238,0.15)", color: "#22d3ee" }}>{tool.method}</span>
                <div className="flex-1 min-w-0">
                ...
                <button
                  onClick={() => setApiTools((p) => p.filter((t) => t.id !== tool.id))}
            ...
            >
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="px-2 py-2 rounded-lg border border-white/[0.08] bg-[#0e0b22] text-xs text-white/70 focus:outline-none"
                >
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button onClick={() => setShowAddAPI(false)}
                  className="px-4 py-2 rounded-lg text-xs text-white/40 border border-white/8 hover:text-white/70 transition-all">
                  Cancel
                </button>
              </div>
            </motion.div>
